...
function loadTodayCalItems(){
	parent.loadRecords("Calories", {onSuccess: function(data){loadTodayCalItemsSucc(data);}, onError: function(){loadCalItemsErr();}});
}
function loadTodaySportsItems(){
	parent.loadRecords("Calories", {onSuccess: function(data){loadTodaySportsItemsSucc(data);}, onError: function(){loadCalItemsErr();}})
}
function loadAllCalItems(){
	parent.loadRecords("Calories", {onSuccess: function(data){loadAllCalItemsSucc(data);}, onError: function(){loadAllCalItemsErr();}});
}
function loadTodayCalItemsSucc(data) {
	...
	$(".recordlist-itemheader").click(function(){ 
		if(typeof(Storage)!=="undefined") {
			...
			localStorage.setItem("hb_backlink", "myApps.html");
		}
		window.top.location = parent.WEB_URL+"detailRecord.html"; 
	});
}
function loadTodaySportsItemsSucc(data) {
	...
	$.each(data.values.records, function(i, item) {
		if(item.app=="caloriesApp" && item.mealtype=="sports"){
			...
			var date = new Date(item.timedate);
			if(moment().isSame(date, 'day')){
				var s = '<li><h3 class="recordlist-itemheader">'+decodeURIComponent(item.name)+'</h3><i>Created at: '+moment(date).format("HH:mm:ss")+'</i><p>Type: '+item.mealtype+': burned ';
				if(item.cal!=undefined){
					burned = burned + parseInt(item.cal);
					s = s+' a total of '+item.cal + ' calories';
				} 
	...
	if(parent.user!=undefined){
		if(parent.user.birthday){
			$("#age").val(Math.floor(moment().diff(moment(parent.user.birthday, "DD-MM-YYYY"), "days")/365));	
		}
...
function addCalEntry(type){
	...
	parent.addRecord(encodeURIComponent(descr), "Calories App Entry", values, {onSuccess: function(data){addCalEntrySucc(data, type);}, onError: function(message){addCalEntryErr(message);}});
}
...
function addBMREntry() {
	...
	parent.addRecord("Daily BMR", "Calories App Entry", values, {onSuccess: function(data){addCalEntrySucc(data, "s");}, onError: function(message){addCalEntryErr(message);}});
}
